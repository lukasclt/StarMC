package com.minecraft.arcade.tiogerson.user.loader;
...
public class UserLoader implements Listener, BukkitInterface {
    ...
    public void onPlayerJoin(PlayerJoinEvent event) {
        ...
        if (user.getRoom() == null) {
            Bukkit.getScheduler().runTaskLater(ArcadeMain.getInstance(), user::lobby, 3L);
            return;
        }
    ...
    public void onPlayerJoin(PlayerSpawnLocationEvent event) {
        ...
        if (target == null) { // Player is searching for the best room through the queue.
            ...
            Room room = ArcadeMain.getInstance().getRoomStorage().get();
            ...
            List<Room> rooms = new ArrayList<>(ArcadeMain.getInstance().getRoomStorage().getRooms().values());
            ...
            while (iterator.hasNext()) {
                ...
                if (iteratorRoom.getStage() != RoomStage.WAITING) {
                    iterator.remove();
                    continue;
                }
    ...
    public void onAsyncPlayerPreLoginEvent(AsyncPlayerPreLoginEvent event) {
        ...
        try {
            try (Jedis redis = Constants.getRedis().getResource()) {
                ...
                redis.del(routeKey);
                ArcadeMain.getInstance().getUserStorage().store(event.getUniqueId(), new User(account, gameRouteContext));
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public void onPlayerQuitEvent(PlayerQuitEvent event) {
        ...
        ArcadeMain.getInstance().getUserStorage().forget(event.getPlayer().getUniqueId());
    }
}