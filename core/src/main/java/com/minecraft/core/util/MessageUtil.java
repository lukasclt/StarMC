...
public enum MessageUtil {
    ...
    private static final int CENTER_MOTD_PX = 132;
    @SuppressWarnings("unused")
    private static final int CENTER_CHAT_PX = 154;

    private MessageUtil(final String s, final int n, final char character, final int length) {
    ...
    public int getBoldLength() {
        if (this == MessageUtil.SPACE) {
            return this.getLength();
        }
        return this.length + 1;
    }

    public static MessageUtil getDefaultFontInfo(final char c) {
        MessageUtil[] values;
        for (int length = (values = values()).length, i = 0; i < length; ++i) {
            final MessageUtil dFI = values[i];
    ...
    public static String makeCenteredMotd(final String message) {
        int messagePxSize = 0;
        boolean previousCode = false;
        boolean isBold = false;
        char[] arrayOfChar;
        for (int i = (arrayOfChar = message.toCharArray()).length, j = 0; j < i; ++j) {
            final char c = arrayOfChar[j];
            if (c == '§') {
                previousCode = true;
                ...
                final MessageUtil dFI = getDefaultFontInfo(c);
                messagePxSize += (isBold ? dFI.getBoldLength() : dFI.getLength());
                ++messagePxSize;
            }
        }
        final int halvedMessageSize = messagePxSize / 2;
        final int toCompensate = 132 - halvedMessageSize;
        final int spaceLength = MessageUtil.SPACE.getLength() + 1;
        int compensated = 0;
    ...
    public static String makeCenteredMessage(final String message) {
        int messagePxSize = 0;
        boolean previousCode = false;
        boolean isBold = false;
        char[] arrayOfChar;
