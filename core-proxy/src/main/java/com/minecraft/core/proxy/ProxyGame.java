...
public class ProxyGame extends Plugin {
    ...
    public void enableBroadcast() {
        broadcasts.add("§eAltere seu idioma principal usando §6/idioma");
        broadcasts.add("§eAcompanhe novidades em nossas redes sociais §6@BlazeMC_BR");
        broadcasts.add("§eAcompanhe suas estatísticas usando §6/stats");
        broadcasts.add("§eVenha fazer parte de nossa comunidade! Acesse nosso discord: §6" + Constants.SERVER_DISCORD);
        broadcasts.add("§eAltere sua experiência de jogo usando §6/prefs");
        getProxy().getScheduler().schedule(this, () -> {
            if (index >= broadcasts.size())
                index = 0;
            getProxy().broadcast(TextComponent.fromLegacyText("§6§l" + Constants.SERVER_NAME.toUpperCase() + " §7» " + broadcasts.get(index)));
            index++;
