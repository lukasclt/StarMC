...
public class Magic extends Kit {
    ...
    @Variable(name = "hg.kit.magic.vector.launch_multiply", permission = Rank.ADMINISTRATOR)
    public double launch_multiply = 1.62;
    @Variable(name = "hg.kit.magic.vector.launch_y_increment", permission = Rank.ADMINISTRATOR)
    public double y_increment = 0.2;

    public Magic(HungerGames hungerGames) {
        super(hungerGames);
        setIcon(Pattern.of(Material.DIAMOND_HOE, true));
        ...
        this.badPotion = new MagicPotionEffect[]{new MagicPotionEffect(false, 16426, buildPotion(MobEffectList.SLOWER_DIG, 300, 0)), new MagicPotionEffect(false, 16426, buildPotion(MobEffectList.SLOWER_MOVEMENT, 400, 0)), new MagicPotionEffect(false, 16388, buildPotion(MobEffectList.POISON, 400, 0)), new MagicPotionEffect(false, 16428)};
    }
    ...
    private void launchPotion(final Player player, final MagicPotionEffect valuePotion) {
        ...
        final Vector multiply = player.getLocation().getDirection().multiply(launch_multiply);
        entityPotion.shoot(multiply.getX(), multiply.getY() + y_increment, multiply.getZ(), 0.75f, 8.0f);
